...
def frequency_penalty(K, coords, areas, members, member_types):
    ...
    try:
        ...
        if np.any(w2 <= 0) or np.isnan(w2).any():
            return 1e9
...
# ------------------- PSO Algorithm -------------------
def pso_algorithm(coords, members, member_types, num_movable_nodes, num_particles=pop_size, num_iterations=num_generations, w=0.5, c1=1.5, c2=1.5):
    particles = []
    velocities = []
    ...
    pbest_scores = np.array([calculate_fitness(p, coords, members, member_types, num_movable_nodes) for p in particles])
    ...
    for iter in range(num_iterations):
        for i in range(num_particles):
            ...
            pos_limit = 0.05
            particles[i][a:] = np.clip(particles[i][a:], coords[:num_movable_nodes].flatten() - pos_limit,
                                       coords[:num_movable_nodes].flatten() + pos_limit)
        ...
        print(f"Iteration {iter+1}: Best Fitness = {gbest_score:.2f}")
...
def plot_truss(nodes, members, areas, member_types, title="Truss Structure", show_weights=True):
    ...
    ax.scatter(nodes[10, 0], nodes[10, 1], nodes[10, 2], color='red', s=90, marker='^', label='External Force 200 N (Nodes 11 & 16)')
    ax.scatter(nodes[15, 0], nodes[15, 1], nodes[15, 2], color='red', s=90, marker='^')
...
if __name__ == "__main__":
    ...
    output_path = "/home/sepehr/VS Code/Project/F- PSO_LoadSet1_Freq.xlsx"
