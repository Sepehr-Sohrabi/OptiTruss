...
def initialize_particles(n_move, coords):
    ...
    for _ in range(pop_size):
        ...
        particles.append(np.concatenate([A, pos.flatten()]))
        velocities.append(np.random.uniform(-v_max, v_max, a + n_move*3))
...
def pso_algorithm(coords, members, member_types, num_movable_nodes):
    ...
    personal_best_scores = np.array([calculate_fitness(p, coords, members, member_types, num_movable_nodes) for p in particles])
    ...
    for gen in range(num_generations):
        for i in range(pop_size):
            ...
            # Clip areas and positions
            particles[i][:a] = np.clip(particles[i][:a], A_min, A_max)
            particles[i][a:] = np.clip(particles[i][a:], coords[:num_movable_nodes].flatten() - 0.05,
                                       coords[:num_movable_nodes].flatten() + 0.05)

            score = calculate_fitness(particles[i], coords, members, member_types, num_movable_nodes)
        ...
        global_best_idx = np.argmin(personal_best_scores)
        global_best = personal_best[global_best_idx].copy()
        best_fitness_history.append(personal_best_scores[global_best_idx])
        print(f"Generation {gen+1}: Weight = {personal_best_scores[global_best_idx]:.2f}")
...
if __name__ == "__main__":
    ...
    output_path = "/home/sepehr/VS Code/Project/F- PSO_LoadSet2_Freq.xlsx"
