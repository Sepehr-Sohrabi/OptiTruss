...
def assemble_mass_matrix(coords, areas, members, member_types):
    ...
    for i, (start, end) in enumerate(members):
        ...
        x1, x2 = coords[n1], coords[n2]
        L = bar_length(x1, x2)
        rho_A = rho * areas[member_types[i]]
        m_local = rho_A * L / 2
        for d in range(3):
            M[n1*3+d, n1*3+d] += m_local
            M[n2*3+d, n2*3+d] += m_local
    return M
...
def frequency_penalty(K, coords, areas, members, member_types, target_freqs=[9,11]):
    ...
    K_mod, _ = apply_boundary_conditions(K.copy(), np.zeros(K.shape[0]), fixed_dofs)
    M_mod, _ = apply_boundary_conditions(M.copy(), np.zeros(M.shape[0]), fixed_dofs)
    try:
        w2, _ = eigh(K_mod, M_mod)
        w = np.sqrt(np.abs(w2))
        f = w / (2*np.pi)
        penalty = sum(max(0, abs(f[i]-target_freqs[i]))*penalty_factor for i in range(len(target_freqs)))
        return penalty
    except:
        return penalty_factor * 100
...
def calculate_fitness(individual, node_coords, members, member_types, num_movable_nodes):
    ...
    freq_penalty = frequency_penalty(K, coords, areas, members, member_types)
    return weight + penalty_factor * overstress_penalty + freq_penalty
...
def pso_algorithm(coords, members, member_types, num_movable_nodes):
    particles = initialize_population(num_movable_nodes, coords)
    velocities = np.random.uniform(-0.01, 0.01, particles.shape)
...
def plot_truss(nodes, members, areas, member_types, title="Truss Structure", show_weights=True):
    ...
    ax.scatter(nodes[1:13,0], nodes[1:13,1], nodes[1:13,2], color='green', s=50, marker='^', label='Load 1500 kg (Nodes 2-13)')
    ax.scatter(nodes[13:37,0], nodes[13:37,1], nodes[13:37,2], color='blue', s=30, marker='^', label='Load 100 kg (Other Nodes)')
...
if __name__ == "__main__":
    ...
    output_path = "/home/sepehr/VS Code/Project/F- PSO_Given Loads_Freq.xlsx"
