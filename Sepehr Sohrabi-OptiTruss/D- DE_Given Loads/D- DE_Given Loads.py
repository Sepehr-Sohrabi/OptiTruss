...
pop_size = 200
num_generations = 200
mutation_rate = 0.1  # این پارامتر DE استفاده نمیشه ولی نگه داشتم
penalty_factor = 1e6
a = 19
n_runs = 1
F_de = 0.8        # عامل مقیاس دهی DE (mutation factor)
CR_de = 0.9       # نرخ کراس‌اور DE
random.seed(42)
np.random.seed(42)
# ------------------------------
...
def differential_evolution(coords, members, member_types, num_movable_nodes):
    ...
    min_generations_before_stop = 100  # حداقل نسل‌ها برای اجازه توقف
    ...
    for gen in range(num_generations):
       ...
       # if prev_best_fitness is not None and gen >= min_generations_before_stop:
       #     rel_change = abs(best_fitness - prev_best_fitness) / prev_best_fitness
       #     print(f"Gen {gen+1}: Best Fitness = {best_fitness:.4f}, Rel. Change = {rel_change:.8f}")
       #     if rel_change < 1e-6:
       #         print(f"\n✅ Converged at generation {gen+1} with rel. change {rel_change:.8f}")
       #         break
       #
...
if __name__ == "__main__":
    ...
    output_path = "/home/sepehr/VS Code/Project/D- DE_Given Loads.xlsx"
