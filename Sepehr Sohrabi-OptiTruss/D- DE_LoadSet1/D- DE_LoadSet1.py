...
a = 19
n_runs = 1
F_scale = 0.9           # جهش بیشتر
CR = 0.9                # نرخ کراس‌اور بیشتر
random.seed(42)
np.random.seed(42)
# ------------------------------
...
def initialize_population(n_move, coords):
    ...
    for _ in range(pop_size):
        ...
        delta = np.random.uniform(-0.05, 0.05, size=(n_move, 3))  # بازه بزرگتر جابجایی نودها
...
def differential_evolution(coords, members, member_types, num_movable_nodes):
    ...
    for gen in range(num_generations):
        for i in range(pop_size):
            ...
            target = pop[i]
            a_vec, b_vec, c_vec = pop[a_], pop[b_], pop[c_]

            mutant = a_vec + F_scale * (b_vec - c_vec)
            mutant[:a] = np.clip(mutant[:a], A_min, A_max)  # محدود کردن مقادیر سطح مقطع
            # محدوده جابجایی نودها را محدود کنید تا از محدوده اولیه بیشتر نشوند:
            n_move = num_movable_nodes * 3
            mutant[a:] = np.clip(mutant[a:], coords[:num_movable_nodes].flatten() - 0.1, coords[:num_movable_nodes].flatten() + 0.1)

            # کراس‌اور
            cross_points = np.random.rand(len(mutant)) < CR
            if not np.any(cross_points):
                cross_points[random.randint(0, len(mutant)-1)] = True
            trial = np.where(cross_points, mutant, target)

            fitness_trial = calculate_fitness(trial, coords, members, member_types, num_movable_nodes)
            fitness_target = calculate_fitness(target, coords, members, member_types, num_movable_nodes)
...
def plot_truss(nodes, members, areas, member_types, title="Truss Structure", show_weights=True):
    ...
    # External force nodes 11 and 16 - red triangles
    ax.scatter(nodes[10, 0], nodes[10, 1], nodes[10, 2], color='red', s=90, marker='^', label='External Force 200 N (Node 11 & 16)')
...
if __name__ == "__main__":
    ...
    output_path = "/home/sepehr/VS Code/Project/D- DE_LoadSet1.xlsx"
